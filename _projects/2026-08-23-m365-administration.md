...
At a small business, email is the number one ticket category. Shared mailboxes, distribution lists, "where did my message go," "why is this in quarantine," "can you give Sarah access to the sales inbox," "somebody shared a file outside the company." None of that is build work, and all of it is the job.

I had zero hands-on with any of it, so this project went straight at the gap – **email, collaboration, and data governance**, in a live tenant, with real failures rather than lab-safe ones.

**The output that matters here is not the configuration.** It is the set of findings, and particularly the ones where a system reported success while doing nothing useful at all.
...
**Starting symptom, known before the project began:** mail sent *from* the domain worked normally, and mail sent *to* the domain never arrived at all.

The domain had been verified in Microsoft 365 months earlier – the ownership TXT record was published and the admin center accepted the domain. That was enough to set a UPN suffix and run Entra Connect, so the work stopped there, which is exactly the kind of half-finished job that waits quietly until somebody tries to send you mail.
...
An inventory script was expected to return `DNS name does not exist` for each of the five missing records, and instead it returned an apparently valid answer for every one of them.
...
**`4.4.1` is a 4.x.x code – transient, not permanent.** The original message was accepted by the sending system on 20 August and the NDR was not issued until 22 August, which is roughly 42 hours of retries before the sender finally gave up on it.
...
The first repair attempt chose Microsoft-managed DNS, which requires delegating the zone. The four Microsoft nameservers were entered into the **DNS record editor** as NS records. Forty-five minutes of waiting produced nothing at all, for the simple reason that nothing had been delegated.
...
*dnsviz.net renders the full delegation chain from root down – the right tool for any real DNSSEC ticket, rather than inferring from resolver behavior.*

**The strategic decision:** abandon Microsoft-managed DNS and keep the zone at the registrar. Four reasons, in order of weight:

1. **It is the skill the job actually requires.** Real clients keep DNS wherever their web developer put it a decade ago, and it stays there. Delegating to Microsoft also means no longer being able to create a record for anything that is not Microsoft.
...
Enabling DKIM was done through the Defender portal rather than the Exchange admin center. That was an unplanned first exposure to that portal, and it is the one Microsoft is consolidating security administration into, so it was time well spent by accident.
...
**One detail worth catching:** across two outbound tests sixteen minutes apart, the SPF-authorized source IP changed. Microsoft rotates outbound sending addresses continuously. **This is exactly why the SPF record uses `include:` and never hardcoded `ip4:` entries.** Anyone who pins Microsoft IPs into SPF has signed up for a silent outage on the day those IPs change.
...
| Authorize | unchanged | as needed | Each sender fixed in SPF or DKIM |
...
Skipping straight to `p=reject` is how you find out on a Monday morning that the client's invoicing system had been sending as their domain from a server nobody ever documented.
...
**Finding – the Submit event is an origin test.** Submit is where a message is handed into transport *by a mailbox inside the organization*. An inbound internet message has no local mailbox submitting it.
...
**Deliver vs Send external** is the other distinction worth keeping straight. Deliver means the message is sitting in a mailbox you administer, and Send external means you successfully handed it to somebody else's server and your visibility ended at that moment.
...
*Default policy state recorded before any modification – the baseline required to put it back.*
...
A real mailbox with a real Entra user object sitting behind it, and **that account is blocked from sign-in**. It is not a distribution group and it is not an alias, which is the confusion worth clearing up first.

**Licensing:** no license required under 50 GB. `info@`, `support@`, `billing@` cost nothing – one of the most useful cost facts in SMB work.

**Security note that gets skipped:** the hidden account is a standing target and the sign-in block is the only control on it. Attackers specifically hunt for shared mailbox accounts where somebody enabled sign-in "to make it easier."

### Three permissions, three storage locations

| Permission | Effect | Retrieved with |
|---|---|---|
| **Full Access** | Open, read, organize, delete. **No right to send.** | `Get-MailboxPermission` |
...
**Three permissions, three unrelated storage locations, three different retrieval methods.** Send As is a directory-level right on the recipient object rather than an ACL on the mailbox store, which is why it needs a different cmdlet entirely. The GUI presents all three as one tidy list, and that is exactly why somebody auditing an inherited tenant misses one.
...
Default behavior sends a delegate's sent copy to **their** Sent Items only. Nobody else on the team sees it – the classic *"we can't tell who already replied, so we keep double-answering"* ticket.
...
A user sent from the shared mailbox using **Send As**, and both the recipient's view and the message trace show nothing but the shared mailbox address. **The submitting user's name appears nowhere in either one.**
...
The contrast turned up four hours later in the same project, entirely by accident. A Task 7 message sent from the same shared mailbox by an account holding **Send on Behalf** showed the shared mailbox at the recipient and the *submitting* account in the trace. Same mailbox, opposite behavior, and the only variable was the delegation type.
...
That is a materially different conclusion from the one the task was set up to teach. Send on Behalf gets recommended *for* accountability all the time; this is a demonstration that the guarantee is weaker than advertised.
...
Coupled objects do not fail at grant time. They fail at *change* time, months later, and they do it silently.
...
**Only universal groups can be mail-enabled.** A standard AGDLP model is built from **global** role groups and **domain-local** resource groups, and **neither one can be mail-enabled as it stands.** That is not a flaw in AGDLP so much as a constraint on the shortcut – "just mail-enable the department group" is not available to you without changing the group's scope first.
...
The cause had nothing to do with group behavior. **The Task 2 anti-spam changes had never been reverted.** What the test actually demonstrated was spam filtering acting on a message that happened to arrive by way of a distribution group.
...
**The dangerous variant of this symptom is not the greyed-out field.** It is a change that appears to save and then **silently reverts on the next sync cycle**. It looks like it worked, and half an hour later it is gone.
...
*One panel, two different strings. One is the mailbox primary SMTP – the reply address. The other, beneath the organization name, is the UPN – the sign-in identity.*

**Separate attributes. Entra Connect syncs both. Editing one never touches the other.**

The user now emails as one string and signs in as another, which is where the ticket *"I changed my email but it says my password is wrong"* comes from. They are typing the new address at the sign-in prompt.
...
The first external share produced no email at all – not in the inbox, not in spam – and no guest object appeared in Entra either.

![The confirmation dialog](/assets/projects/m365-administration/48-people-picker-trap.png)

The break-glass Global Administrator account carried a display name identical to the intended external recipient's. **The people picker resolved to it.** The file had been shared internally, with an emergency access account, and the dialog had said exactly that at the time.
...
**Knowing which tool does not apply is worth as much as knowing which one does.** Somebody who traces for twenty minutes and turns up nothing walks away concluding the system is broken.
...
**This is not "Anyone" behavior.** Anonymous links require no sign-in, create no directory object, and issue no verification code at all, so a named-recipient share is a **different mechanism** operating at a lower minimum requirement:
...
*Closing state: SharePoint returned to New and existing guests, OneDrive to organization-only.*

**Access finding:** the *Manage access* surface required the **SharePoint Administrator** role, and the correct response to that is granting the one role you diagnosed rather than a stack of them. The production answer to standing admin roles is **Privileged Identity Management** – eligible rather than active, elevated just-in-time. **PIM requires Entra ID P2 and Business Premium includes P1**, so it is not available on this SKU.
...
**Two separate operations in two separate places.** Creation happens under Information Protection → Sensitivity labels and publication happens under **Label policies**, and a label that exists without being published appears nowhere at all – which is the single most common *"I created a label and nobody can see it"* ticket.
...
The trace showed **three DLP rule evaluations**, the client showed a policy tip before send, and the sender received a notification reading *"You shared content that contains 1 or more credit card numbers with people outside your organization."*
...
An administrator who deployed this and walked away would believe the tenant was protected. It was not, and nothing in the console would have said otherwise.

### Finding – Encrypt is not Block

Note that the enforcement trace **still ends in Send external.** Encrypt wraps the message and delivers it, and the recipient authenticates to open it, which means the data still leaves the organization – protected in transit and at rest, but not withheld.

**If the requirement is "this must not leave," the action is Block.** Clients conflate the two constantly, and the distinction is the whole conversation once compliance is in the room.
...
**Two notification paths, two audiences.** The user notification (*"**You** shared content…"*) is an entirely different mechanism from the `Send alerts to Administrator` rule action, which means receiving one of them is no evidence at all that the other one fired.
...
The three most valuable findings were not specific to any one product. Each turned up in a different service, and taken together they read as doctrine rather than as trivia.
...
A configuration override does not only change behavior. It changes what your tests *report*, and it does that silently. The test still runs, it still returns a result, and the result still looks like an answer.
...
4. **In an inherited environment, inventory the overrides before trusting any diagnostic.** Wildcard DNS records, allow lists, disabled and/or simulation-mode policies are all invisible until somebody goes looking for them specifically.
...
**When two things change at once, causality is gone**, and in a client environment that is the difference between a root cause and a guess that gets repeated later.
...
Recorded here rather than glossed over and/or quietly dropped.
...
| Sign-in with a non-UPN proxy address | Observed but not cleanly tested – likely a cached session plus OWA display behavior. Requires a cold-session test. **Unverified.** |
...
**Exchange Online** – message trace and event-chain interpretation, Message-ID vs MessageTrace ID, mail flow origin analysis, shared mailboxes, Full Access / Send As / Send on Behalf and their three separate storage locations, automapping, sent-items behavior for delegates, distribution groups vs mail-enabled security groups, group expansion tracing, `proxyAddresses` and primary SMTP promotion, Exchange Online PowerShell
...
6. Smaller project scopes. Both projects so far have run long enough that verification discipline degraded toward the end, and that is a process problem rather than a knowledge problem

---

## Screenshots

The full capture set, ordered by task. Any image opens full size when clicked.
...
<div class="gallery-box">
  ...
  <em>Full capture set, ordered by task – any image opens full size when clicked</em>
</div>
