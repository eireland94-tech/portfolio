...
### `[DOCTRINE-13]` A search that returns only aggregators has told you the company is dead
...
### `[DOCTRINE-14]` Physical inspection answers what documents cannot
...
### `[DOCTRINE-15]` A cloud migration's real savings for a small organization is avoided capital spend, not a recurring discount
...
### `[FILE-10]` Migrating an on-premises file share to SharePoint/OneDrive — tooling and sequencing
...
### `[M365-21]` Assessing security-tool overlap before recommending a cancellation
...
### `[WIN-08]` Windows edition capabilities and product lifecycle
...
### `[WIN-09]` Admin console quick reference — snap-ins and their file names
...
### `[DISK-04]` RAID levels — and the misconception that costs data
...
### `[DISK-05]` Cross-platform file system recognition
...
### `[SOP-13]` Malware removal — the sequence, not just the scan
...
### `[SOP-14]` Change management vocabulary
...
### `[SOP-15]` Backup strategy — types and the 3-2-1 rule
...
### `[SOP-16]` Physical safety, environmental controls, and disposal
...
### `[SOP-17]` Regulated data — the acronyms a client conversation assumes you know
...
### `[SOP-18]` Client communication essentials
...
### Mail flow — third-party gateways in front of Microsoft 365
...
### Site survey and vendor assessment
...
### `[SYSF-07]` Domain vs. workgroup, and the vocabulary the operational sections assume
...
### `[DNS-MAIL-09]` Third-party mail gateways in front of Microsoft 365 — reading the topology
...
### `[EXO-15]` Enhanced Filtering for Connectors — the fix a gateway topology needs, and the failure mode if it's skipped
...
## 31 — PRINTERS AND IMAGING `[PRN]`

### `[PRN-01]` The four printer technologies — do not cross-diagnose between them
...
### `[PRN-02]` The laser imaging process, in order
...
### `[PRN-03]` Symptom → cause, laser printers
...
### `[PRN-04]` Paper path faults — a different failure class than print-quality faults
...
### `[PRN-05]` 🛑 Do not confuse a queue fault with a mechanical fault
...
### `[PRN-06]` 🛑 DANGER — never compressed air or a standard vacuum on toner
...
### `[PRN-07]` Network printer connectivity triage
...
### `[MAC-01]` Windows → macOS translation
...
### `[MAC-02]` File system layout — mind the two Library folders
...
### `[MAC-03]` File types and uninstall behavior
...
### `[MAC-05]` Patching, including out-of-band security response
...
## 33 — PHYSICAL LAYER AND CONNECTORS `[CONN]`

### `[CONN-01]` Cable standards
...
### `[CONN-02]` Connector identification — what actually turns up in the field in 2026
...
### `[CONN-03]` Video signal capability — "why is there no sound through my monitor"
...
### `[SITE-01]` Why a survey pays for itself
...
### `[SITE-02]` Permission and standing
...
### `[SITE-04]` Technique, which decides whether the trip was worth anything
...
### `[SITE-05]` Evidence handling — the rule that protects you
...
### `[SITE-06]` Turning a service tag into manufacturer-stated fact
...
## 35 — VENDOR AND PLATFORM DUE DILIGENCE `[VEND]`

### `[VEND-01]` Is the platform alive? — reading a product line's real state
...
### `[VEND-02]` Should you learn a declining platform?
...
### `[VEND-03]` Reading what an incumbent actually delivers
